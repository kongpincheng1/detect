...
class YOLOv5ROS2(Node):
    def __init__(self):
        ...
        # self.model = torch.hub.load('ultralytics/yolov5', 'yolov5s', source='github')
        self.model.eval()
    ...
    def pixel_to_world(self, u, v, depth):
        ...
        """
        # 假设相机内参已知，替换为你的标定结果
        fx, fy = 604.7058715820312, 603.9238891601562  # 焦距 (单位：像素)
        cx, cy = 321.9357604980469, 248.05108642578125  # 主点 (单位：像素)
        X = (u - cx) * depth / fx
        Y = (v - cy) * depth / fy
        Z = depth
        return X, Y, Z
    ...
    def should_update_target(last_x, last_y, current_x, current_y, threshold=0.01):
        """
        判断目标点是否需要更新。
    ...
    def limit_target_step(last_x, last_y, target_x, target_y, max_step=0.15):
        """
        限制目标点的相邻两次发布的最大移动距离。
    ...
    def process_and_publish(self, X, Y, Z):
        ...
        if self.last_filtered_x and self.last_filtered_y:
            X_limit, Y_limit, Z_limit = self.limit_target_step(self.last_filtered_x, self.last_filtered_y, X, Y)
        else:
            X_limit = X
            Y_limit = Y
        # 对 X、Y、Z 分别应用平滑滤波
        ...
        point_msg = Point(x=self.last_filtered_x, y=self.last_filtered_y, z=self.last_filtered_z)
